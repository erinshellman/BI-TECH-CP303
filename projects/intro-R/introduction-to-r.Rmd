...
ff = read.delim('~/projects/BI-TECH-CP303/datasets/forest_fires.tsv',
                header = TRUE,
                sep = '\t')
```
...
analyze data with R. We're going to use many of his packages in this class to 
solve complex problems and at times it might be frustrating, but you'll be a pro
in no time.
...
# define a variable called 'value'
value = (4.59 / 0.1)^3
print(value)

# R will even do these operations element-wise on vectors!
vector1 = c(1, 2, 3, 4)
...
```{r udf}
# make input
my_friends = c('Gary', 'Wendy', 'Uncle Brian')
...
df = read.delim('path/to/your/data/data.file',
                header = TRUE,
                sep = '\t')
```
...
the data you'll be working with in the first project. TODO: UPDATE. The data are from Washington
...
data = read.delim('/your/dir/forest_fires.tsv',
                  header = TRUE,
                  sep = '\t')
```
