...
We can convert the *relationship* vector is a special categorical type called a 
...
Using data of the wrong type is a common sources of errors. Luckily there are 
ways to learnt the type of an object if you encounter problems.
...
```{r ggpairs extended example, eval = F, include = FALSE}
data$season = factor(data$season, levels = c('Winter', 'Spring', 'Summer', 'Autumn'))
ggpairs(data[ , c('bar', 'restaurant', 'duration_mean', 'num_rentals', 'season')],
        colour = 'season')
```
...
```{r assign2A ggpairs , include = F, eval = F, echo = F}
ggpairs(data[ , c('tourism_artwork', 'railway_level_crossing', 'duration_mean', 'num_rentals')])
```
<!---
