...
You might have noticed that one of the seasons is missing. By default R choses a 
...
The interpretation of the new model is that stations without crosswalks or parking have an average of 2,687 rentals in the spring. Those same stations can expect an additionanl 232 rentals in the summer, and about 906 less in the fall and 2,116 less in the winter.
...
I'm uisng the `select` verb from `dplyr` to remove variables that aren't 
...
For example the dataset contains a variable for *nightclub*, *pub* and *bar*, 
